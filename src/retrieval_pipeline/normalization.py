...
class ParsingBasedNormalizer:
    ...
    def _get_subj_obj_from_dep(self, skeleton_dependency):
        ...
        for h, rel, t in skeleton_dependency:
            if rel.startswith('nsubj'): subjs.append(t)  # nsubj, nsubjpass
    ...
    def process_info(self, info: dict):
        ...
        skeleton_dep = info['skeleton_dependencies'] if 'skeleton_dependencies' in info else info['dependencies']

        subjs, objs = self._get_subj_obj_from_dep(skeleton_dep)
        compounds, modifiers, cases = self._get_data_from_dep(dependency)
        if subjs is None: return None
        ...
        for subj in subjs:
            span, details, words = self._restore_span(subj, info['words'],
                                                    compounds, modifiers)
            returns['subjs'].append((span, details, words))
    ...
    def get_personal_words(self, info: dict):
        ...
        subj_list, obj_list, poss_list, nmod_list = processed_info.get('subjs', []), processed_info.get('objs', []),\
