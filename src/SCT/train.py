...
model_collator_map = {
    'graph': (GraphMCModel, GraphCollator),
    'text-graph': (TextGraphMCModel, TextGraphCollator)
    # 'text-graph': (TextGraphMCModel_Att, TextGraphCollator)
}
...
def main(rank, world_size, configs):
    init_process_group(rank, world_size, timeout=600)
    args = Arguments(
        use_ddp=True,
        rank=rank,
    ...
    datasets = GraphMCDataset.make_datasets(args.seed, args.dataset, graph_name=args.graph_name, add_rev_edges=args.add_rev_edges)
    ...
    model = model(args, num_choices=2)
...
parser.add_argument("--name", type=str, default='story-cloze-test')
